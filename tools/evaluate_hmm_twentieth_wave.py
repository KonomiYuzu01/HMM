...
def main() -> None:
    ...
    evaluation.OUTPUT = Path("output/hmm_twentieth_wave_validation")
    evaluation.CANDIDATES = {
        "momentum_plus_volatility_acceleration": {
            "normal": Path(
                "output/research_hmm_momentum_plus_volatility_acceleration_restarts5"
            ),
            "proxy": Path(
                "output/research_hmm_momentum_plus_volatility_acceleration_restarts5_20y_proxy"
            ),
        }
    }
    evaluation.main()
