...
OUTPUT = Path("output/r10_dynamic_capital_efficiency")
MINIMUM_FRACTION = 0.25
MAXIMUM_FRACTIONS = (0.25, 0.50, 0.75, 1.00)
GUARD_CAP = 0.15
    ...
    for sample, settings in samples.items():
        ...
        for scenario, (
            ...
            for maximum_fraction in MAXIMUM_FRACTIONS:
                ...
                if scenario == "primary":
                    label = int(round(maximum_fraction * 100))
                    candidate.to_csv(
                        OUTPUT / f"{sample}_max{label}_daily.csv",
                        index_label="date",
                    )
