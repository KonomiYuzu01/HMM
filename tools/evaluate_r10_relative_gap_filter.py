...
OUTPUT = Path("output/r10_relative_gap_filter")
RELATIVE_FILTERS = (None, -0.0025, -0.0050, -0.0075, -0.0100)
GUARD_CAP = 0.15
SUBSTITUTION_FRACTION = 0.25


def filter_label(relative_filter: float | None) -> str:
    return (
        "none"
        if relative_filter is None
        else f"{int(round(abs(relative_filter) * 10_000))}bp"
    )
...
def main() -> None:
    ...
    for sample, settings in samples.items():
        ...
        for scenario, (
            ...
            for relative_filter in RELATIVE_FILTERS:
                guard_daily, guard_weights = simulate_guard(
                    ...
                    selected_guard(
                        ...
                        relative_gap_trigger=relative_filter,
                    ),
                    start_date=start,
                )
                ...
                if scenario in {"primary", "current_liquidity"}:
                    label = filter_label(relative_filter)
                    scenario_suffix = (
                        ""
                        if scenario == "primary"
                        else "_current_liquidity"
                    )
                    candidate.to_csv(
                        OUTPUT
                        / (
                            f"{sample}_rel{label}"
                            f"{scenario_suffix}_daily.csv"
                        ),
                        index_label="date",
                    )
