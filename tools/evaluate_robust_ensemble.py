...
DESTINATION = OUTPUT_ROOT / "robust_vol_guarded_floor_validation"
BASELINE = "paper_core_growth"
CANDIDATE = "paper_core_robust_vol_guarded_floor_ensemble"
BASELINE_COST15 = "paper_core_growth_cost15"
CANDIDATE_COST15 = "paper_core_robust_vol_guarded_floor_ensemble_cost15"
BASELINE_2012 = "paper_core_growth_2012"
CANDIDATE_2012 = "paper_core_robust_vol_guarded_floor_ensemble_2012"
PERIODS = {
        ...
        [
            {
                ...
                "cost15_cagr_delta": candidate_stress["cagr"]
    ...
    annual = pd.concat(
        {
            ...
            "ensemble": daily[CANDIDATE].loc[sample, "net_return"],
            "baseline_cost15": daily[BASELINE_COST15].loc[sample, "net_return"],
            "ensemble_cost15": daily[CANDIDATE_COST15].loc[sample, "net_return"],
        },
        axis=1,
    )
    annual = (1.0 + annual).groupby(annual.index.year).prod() - 1.0
    annual["ensemble_minus_baseline"] = annual["ensemble"] - annual["baseline"]
        ...
        [
            {
                ...
                "identical_daily_return_share": float(
                    np.isclose(
                        common["start_2015"], common["start_2012"], atol=1e-12
                    ).mean()
                ),
        ...
        [
            {
                "annual_wins_vs_baseline": int(
                    (annual["ensemble_minus_baseline"] > 0.0).sum()
                ),
                ...
                "annualized_outer_cost": float(
                    daily[CANDIDATE].loc[sample, "outer_cost"].mean() * 252
                ),
                "cost15_annualized_total_cost": float(
