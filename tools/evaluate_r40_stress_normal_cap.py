...
OUTPUT = Path("output/r40_stress_normal_cap")
STRESS_NORMAL_CAPS = (0.90, 1.00, 1.10)


@dataclass(frozen=True)
class StressNormalCapCandidate:
    name: str
    stress_normal_non_cash_cap: float
...
def main() -> None:
    ...
    candidates = tuple(
        StressNormalCapCandidate(
            f"stress_normal_cap_{int(cap * 100):03d}", cap
        )
        for cap in STRESS_NORMAL_CAPS
    )
            ...
            rows.append(
                {
                    ...
                    "stress_normal_non_cash_cap": candidate.stress_normal_non_cash_cap,
                    "sample": sample,
