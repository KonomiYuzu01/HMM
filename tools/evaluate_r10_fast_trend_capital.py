...
OUTPUT = Path("output/r10_fast_trend_capital")
...
def main() -> None:
    ...
    signal_rows: list[dict[str, float | int | str]] = []
    for sample, settings in samples.items():
        ...
        for candidate_name, signal in trend_fraction.items():
            if signal is not None:
                selected_signal = signal.loc[signal.index >= start]
                signal_rows.append(
                    {
                        "sample": sample,
                        "candidate": candidate_name,
                        "active_share": float(
                            selected_signal.gt(0.0).mean()
                        ),
                        "latest_fraction": float(
                            selected_signal.iloc[-1]
                        ),
                    }
                )
        for scenario, (
            ...
            for candidate_name, signal in trend_fraction.items():
                maximum_fraction = {
                    "fixed_40pct": 0.40,
                    "fast_trend_75pct": 0.75,
                    "fast_trend_100pct": 1.0,
                }[candidate_name]
                candidate = simulate_gde_substitution(
                    ...
                    substitution_fraction_override=signal,
                )
    ...
    signals.to_csv(OUTPUT / "signal_diagnostics.csv", index=False)
