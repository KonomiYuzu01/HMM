...
OUTPUT = Path("output/r10_guard_trigger_ridge")
ABSOLUTE_GAPS = (-0.0250, -0.0275, -0.0300)
MINIMUM_WEIGHTS = (0.50, 0.55, 0.60)
GUARD_CAP = 0.15
...
def main() -> None:
    ...
    for sample, settings in samples.items():
        ...
        for scenario, (
            ...
            for gap in ABSOLUTE_GAPS:
                for minimum_weight in MINIMUM_WEIGHTS:
                    ...
                    if scenario == "primary":
                        gap_label = int(round(abs(gap) * 10_000))
                        weight_label = int(round(minimum_weight * 100))
                        candidate.to_csv(
                            OUTPUT
                            / (
                                f"{sample}_gap{gap_label}bp"
                                f"_min{weight_label}_daily.csv"
                            ),
                            index_label="date",
                        )
    ...
    summary = metrics.loc[
        metrics.apply(
            lambda row: row["period"] == full_period[row["sample"]],
            axis=1,
        )
    ]
    print(
