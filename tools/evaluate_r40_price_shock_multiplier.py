...
OUTPUT = Path("output/r40_price_shock_multiplier")
CALM_MULTIPLIERS = (20.0, 30.0, 50.0)
...
def causal_price_stress_signals(
    closes: pd.DataFrame,
    *,
    five_day_stress_threshold: float,
) -> pd.DataFrame:
...
def simulate_price_candidate(settings, scenario, components, candidate):
    original = shock.causal_stress_signals

    def price_only(closes, *, five_day_stress_threshold):
        return causal_price_stress_signals(
            closes,
            five_day_stress_threshold=five_day_stress_threshold,
        )

    shock.causal_stress_signals = price_only
    try:
...
def main() -> None:
    ...
    for candidate in candidates:
        ...
        for sample, settings in settings_by_sample.items():
            trial, diagnostics = simulate_price_candidate(
                settings,
                scenario,
