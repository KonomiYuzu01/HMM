...
from tools.evaluate_r40_sparse_shock_multiplier import (
    causal_sparse_stress_signals,
)


OUTPUT = Path("output/r40_sparse_shock_strength")
STRESS_MULTIPLIERS = (9.5, 10.0, 11.0, 12.0)
ONE_DAY_STRESS_THRESHOLD = -0.05
...
class SparseShockStrengthCandidate:
    name: str
    bear_multiplier: float
    calm_multiplier: float = CALM_MULTIPLIER
    one_day_stress_threshold: float = ONE_DAY_STRESS_THRESHOLD
    five_day_stress_threshold: float = FIVE_DAY_STRESS_THRESHOLD
...
def simulate_strength_candidate(settings, scenario, components, candidate):
...
def main() -> None:
    ...
    candidates = tuple(
        SparseShockStrengthCandidate(
            f"stress_multiplier_{str(multiplier).replace('.', '_')}",
            multiplier,
        )
            ...
            rows.append(
                {
                    ...
                    "stress_multiplier": candidate.bear_multiplier,
                    "sample": sample,
