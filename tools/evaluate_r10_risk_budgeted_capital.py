...
OUTPUT = Path("output/r10_risk_budgeted_capital")
GDE_NO_TRADE_BAND = 0.0025
INCREMENTAL_VOLATILITY_BUDGET = 0.01
TRACKING_ERROR = 0.091468
...
def main() -> None:
    ...
    for sample, settings in samples.items():
        ...
        for scenario, (
            ...
            risk_fraction = causal_risk_budgeted_substitution_fraction(
                guard_weights,
                closes,
                maximum_fraction=1.0,
                incremental_volatility_budget=(
                    INCREMENTAL_VOLATILITY_BUDGET
                ),
                lookback_days=60,
                annual_tracking_error=TRACKING_ERROR,
            )
            trade_dates = guard_daily.index[
                guard_daily["turnover"].gt(1e-14)
            ]
            active_fraction = risk_fraction.reindex(trade_dates).dropna()
            fraction_rows.append(
                {
                    "sample": sample,
                    "scenario": scenario,
                    "observations": len(active_fraction),
                    "mean_fraction": float(active_fraction.mean()),
                    "median_fraction": float(active_fraction.median()),
                    "fraction_p10": float(
                        active_fraction.quantile(0.10)
                    ),
                    "fraction_p90": float(
                        active_fraction.quantile(0.90)
                    ),
                    "maximum_fraction": float(active_fraction.max()),
                }
            )
            candidates = {
                "fixed_40pct": (0.40, None),
                "risk_budgeted_1pp": (1.0, risk_fraction),
            }
            for candidate_name, (
                maximum_fraction,
                fraction_override,
                ...
                candidate = simulate_gde_substitution(
                    ...
                    substitution_fraction_override=(
                        fraction_override
                    ),
                )
                ...
                if scenario == "current_liquidity":
                    current_candidates[(sample, candidate_name)] = (
                        candidate
                    )
                    candidate.to_csv(
                        OUTPUT
    ...
    fractions = pd.DataFrame(fraction_rows).drop_duplicates(
        subset=["sample"]
    )
    fractions.to_csv(OUTPUT / "fraction_diagnostics.csv", index=False)
    ...
    tracking_rows = []
    for candidate_name in ("fixed_40pct", "risk_budgeted_1pp"):
    ...
    tracking.to_csv(OUTPUT / "tracking_comparison.csv", index=False)
    full_periods = {
        "complete_2015_2026",
        "complete_2006_2026",
        "live_2022_2026",
    ...
    print("\nRisk-budget fraction diagnostics:")
    print(fractions.round(6).to_string(index=False))
    print("\nTracking comparison:")
