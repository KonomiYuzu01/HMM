...
def test_current_close_cannot_change_current_open_target() -> None:
    ...
    before, _, _ = volatility_conditioned_schedule(
        weights,
        daily,
        closes,
    )
    ...
    after, _, _ = volatility_conditioned_schedule(
        weights,
        daily,
        changed,
    )
...
def test_high_volatility_never_uses_more_than_low_volatility() -> None:
    closes, weights, daily = _inputs()
    ...
    high = diagnostics["high_volatility_active"]
    assert high.any()
    assert diagnostics.loc[
        high, "state_active_multiplier"
    ].eq(1.30).all()
    assert diagnostics.loc[
        ~high, "state_active_multiplier"
    ].eq(1.35).all()
...
def test_cash_and_non_cash_limits_hold() -> None:
    closes, weights, daily = _inputs()
    adjusted, _, diagnostics = volatility_conditioned_schedule(
        weights,
        daily,
        closes,
    )
