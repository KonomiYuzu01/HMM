...
from tools.evaluate_r35_stronger_concave_trend_risk import (
    ACTIVE_MULTIPLIER,
    CASH_FLOOR,
    _configure_helpers,
)
...
def test_r35_uses_130_percent_but_caps_absolute_risk() -> None:
    _configure_helpers()
    ...
    assert ACTIVE_MULTIPLIER == 1.30
    assert np.allclose(
        diagnostics.loc[
            permitted, "implemented_non_cash_weight"
        ],
        min(0.80 * 1.070 * ACTIVE_MULTIPLIER, 1.20),
    )
